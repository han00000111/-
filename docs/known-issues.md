...
- 2026-06-22 已按 `delivery-checklist.md` 完成 `1920×1080`、`1366×768`、`1024×768` 人工页面验收。
- 当前仍没有可重复执行的截图基线或端到端视觉测试。
- 影响：后续样式改动仍需重新执行人工分辨率检查。

### 窄屏宽表依赖卡片内横向滚动

- 设备明细、指令回执、日志审计等字段较多的表格，在 `1366×768`、`1024×768` 下使用卡片内横向滚动查看完整列。
- 影响：页面和卡片不会被撑破，但窄屏无法同时看到全部字段；本轮不删减业务列。
