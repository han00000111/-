...
export function OverviewPage({
  ...
  const overviewDevices = runtime.devices ?? [];
  const overviewTasks = runtime.tasks ?? [];
  const overviewAlarms = runtime.alarms ?? [];
  ...
  const jumpFromAlarm = (alarm) => {
    ...
    if (alarm.jumpTarget === 'devices') {
      setSelectedDeviceId(alarm.deviceId ?? alarm.device);
      setPage('devices');
      return;
    }
    setPage('alarms');
...
function OverviewStatusBar({ alarms: alarmRows, devices: deviceRows, setPage, tasks: taskRows }) {
  ...
  const deviceStats = derivedDeviceStats;
  const taskStats = derivedTaskStats;
...
function getAlarmKey(alarm, index) {
  return [
    ...
    alarm.deviceId || alarm.device || alarm.source || alarm.object || 'unknown',
    alarm.type || 'type',
    alarm.createdAt || alarm.updatedAt || alarm.time || 'time',
    index,
  ].join('-');
}
      ...
      {sortedAlarms.map((alarm, index) => (
        <button type="button" key={getAlarmKey(alarm, index)} className={`exception-item ${toneOf(alarm.level)} ${alarm.status === '已恢复' ? 'resolved' : ''}`} onClick={() => onAlarmJump(alarm)}>
          ...
          <span className="exception-main">
            ...
            <span className="exception-meta">{alarm.deviceId ?? alarm.device}｜{alarm.type}</span>
          </span>
          <span className="exception-side">
            ...
            <span className="exception-time">{alarm.createdAt ?? alarm.updatedAt ?? alarm.time}</span>
          </span>
        </button>
      ))}
      {brokenInterlocks.map((it) => (
        <button type="button" key={`il-${it.id ?? it.name}-${it.deviceId ?? it.device}`} className="exception-item warn" onClick={() => setPage('alarms')}>
          ...
          <span className="exception-main">
            ...
            <span className="exception-meta">{it.deviceId ?? it.device}</span>
          </span>
          <span className="exception-side">
            ...
            <span className="exception-time">{it.updatedAt ?? it.createdAt ?? it.time}</span>
          </span>
        </button>
      ))}
    </div>
...
function DeviceHealthGrid({ deviceRows, onSelect, setPage }) {
  const groups = useMemo(() => {
    ...
    deviceRows.forEach((d) => {
      const onlineStatus = d.onlineStatus ?? d.online;
      const g = map.get(d.type) ?? { type: d.type, total: 0, online: 0, offline: 0, abnormal: 0, firstAbnormalId: null };
      g.total += 1;
      if (onlineStatus === '在线') g.online += 1;
      if (onlineStatus === '离线') g.offline += 1;
      if (d.alarmCount > 0) {
        g.abnormal += 1;
...
function RecentActivity() {
  ...
  return (
    <div className="recent-activity">
      {rows.map((row, idx) => (
        <div className="activity-item" key={`${row.id ?? row.createdAt ?? row.time}-${idx}`}>
          <span className={`activity-tag ${(row.type ?? row.logType) === '报警' ? 'bad' : (row.type ?? row.logType) === '指令' ? 'accent' : 'muted'}`}>{row.type ?? row.logType}</span>
          <span className="activity-content">{row.message ?? row.content}</span>
          <span className="activity-time">{row.createdAt ?? row.updatedAt ?? row.time}</span>
        </div>
      ))}
    </div>
  );
